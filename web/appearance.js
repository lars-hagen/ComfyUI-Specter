...
app.registerExtension({
    ...
    settings: [
        { id: "Specter.GoogleWarning", category: ["Specter", "Status", "Google"], name: "Google Services", tooltip: "Google connectivity check", sortOrder: 100,
            type: () => { const el = document.createElement("div"); el.innerHTML = '<span style="color: #888; font-weight: 500;">Checking...</span>'; fetch("/specter/health").then(r => r.json()).then(d => { if (d.google_blocked) { el.innerHTML = '<span style="color: #fbbf24; font-weight: 500;">⚠ Google services blocked from server</span><br><span style="color: var(--p-text-muted-color, #888); font-size: 12px;">The ComfyUI server cannot reach www.gstatic.com.<br>Gemini will not work.<br>Google login on Grok/ChatGPT won\'t work.<br>Export cookies from your local browser and use cookie import instead.</span>'; } else { el.innerHTML = '<span style="color: #4ade80; font-weight: 500;">✓ Google services accessible</span>'; } }).catch(() => { el.innerHTML = '<span style="color: #888; font-weight: 500;">Unable to check</span>'; }); return el; }, defaultValue: "" },
        { id: "Specter.BrowserStatus", category: ["Specter", "Status", "Browser"], name: "Browser Status", tooltip: "Shows if the browser automation is ready", sortOrder: 10,
            type: () => { const el = document.createElement("span"); el.textContent = "Checking..."; el.style.color = "#888"; el.style.fontWeight = "500"; fetch("/specter/health").then(r => r.json()).then(d => { el.textContent = d.ready ? "Ready" : "⚠ " + (d.error || "Not ready"); el.style.color = d.ready ? "#4ade80" : "#f87171"; }).catch(() => { el.textContent = "⚠ Unable to check"; el.style.color = "#fbbf24"; }); return el; }, defaultValue: "" },
